...
MarkerOverlappingRecognizer.prototype = {
  ...
  getOverlapRect: function (marker) {
    ...
    var offset = proj.fromCoordToOffset(position)
    var tolerance = this._options.tolerance || 3
    var rectLeftTop = offset.clone().sub(tolerance, tolerance)
    var rectRightBottom = offset.clone().add(tolerance, tolerance)
  ...
  _onOver: function (e) {
    ...
    var map = this.getMap()
    var proj = map.getProjection()
    var offset = proj.fromCoordToOffset(marker.getPosition())
    var overlaped = this.getOverlapedMarkers(marker)
    ...
    if (overlaped.length > 1) {
      if (this._options.highlightRect) {
        ...
        var max = proj.fromOffsetToCoord(bounds.getMax())
  ...
  _renderIntersectList: function (overlaped, offset) {
    ...
    var template = this._options.intersectListTemplate
    var itemTemplate = null
    var itemPlace = null
    var matches = REPEAT_REGEX.exec(template)
    var uid = this._guid()
    var self = this
